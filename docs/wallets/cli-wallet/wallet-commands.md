title: Gyuanx Documentation | Gyuanx, Oxen and Monero CLI Command Guide
description: The Gyuanx CLI wallet is a wallet first developed by Monero. Gyuanx has added additional commands ontop of the monero-wallet-cli and this document goes through all the Gyuanx and Monero CLI commands available within the gyuanx-wallet-cli.

# Gyuanx CLI Guide

The Gyuanx CLI wallet is a wallet copied from oxen developers. Gyuanx has added additional commands ontop of the `monero-wallet-cli` and this document goes through all the Gyuanx and Monero CLI commands available within the `gyuanx-wallet-cli`.


## Displaying commands

The `gyuanx-wallet-cli` has multiple commands to conduct different operations on the Gyuanx Blockchain. By typing `help` and clicking enter after loading your wallet will bring up the commands that can be used.
...
```
[wallet L4Fb5e]: account new Secondary account
[wallet L4Fb5e]: Untagged accounts:
[wallet L4Fb5e]: Account Balance Unlocked balance Label
[wallet L4Fb5e]: 0 L4Fb5e 0.000000000 0.000000000 Primary account
[wallet L4Fb5e]: * 1 LY5J5W 0.000000000 0.000000000 Secondary account
[wallet L4Fb5e]: ----------------------------------------------------------------------------------
[wallet L4Fb5e]: Total 0.000000000 0.000000000
```
...
```
[wallet L4Fb5e]: account switch 0
[wallet L4Fb5e]: Currently selected account: [0] Primary account
[wallet L4Fb5e]: Tag: (No tag assigned)
[wallet L4Fb5e]: Balance: 0.000000000, unlocked balance: 0.000000000
```

### 1.3 Changing account labels

To change the label name connected to a specific Gyuanx Primary or Sub-address use the command:
...
The `gyuanx-wallet-cli` allows you to group accounts by tagging or untagging them.
...
Reserve Proofs are used to generate a signature proving that you own an amount of Gyuanx, with the option to sign the reserve proof with a key.

For example let’s imagine you see a car for sale but they will accept Gyuanx as payment, however they have advised in their online listing that they are only interested in serious buyers and require you to prove you have the Gyuanx within your initial contact. Luckily we can use the Reserve Proof commands for this proof.
...
If the individual you are sending this proof to requires you to prove you have 1000 Gyuanx you will need to replace the section `(all|<amount>)` with a 1000, otherwise replace it with the amount you need to prove you have reserved. If you want to put an extra layer of encryption over the file replace `[<message>]` with a password.
...
Wallet password:
signature file saved to: gyuanx_reserve_proof
```

This signature file `gyuanx_reserve_proof` will be saved in your Gyuanx folder, where your daemon and wallet keys are. Keep in mind every time you run the `get_reserve_proof` command it will overwrite your `gyuanx_reserve_proof` file.

You will want to send this file to the person who requires the proof. You can upload the `gyuanx_reserve_proof` file through [https://transfer.sh/](https://transfer.sh/) by running the command within the folder of your signature file:

```
curl --upload-file ./gyuanx_reserve_proof https://transfer.sh/gyuanx_reserve_proof`
```
...
```
https://transfer.sh/QhoC7/gyuanx_reserve_proof
```

Make sure you provide the following to the individual who will be checking your reserve proof:

-   The gyuanx_reserve_proof file through the transfer.sh link.

-   The gyuanx address you are proving has Gyuanx in it.
    
-   The `<message>` if you encrypted the file.

### 5.2 Checking Reserve Proof

To check a reserve proof we need to first have the `gyuanx_reserve_proof` file in our Gyuanx folder.

If you do not have the `gyuanx_reserve_proof` file in your gyuanx folder request the individual sending the file to you to use [https://transfer.sh/](https://transfer.sh/), once they send you the link to their `gyuanx_reserve_proof` you can use the following command to download it.

```
curl <link> -o gyuanx_reserve_proof
```

Replacing `<link>` with the link to download the `gyuanx_reserve_proof`.

Now that the `gyuanx_reserve_proof` is in our folder we can run the following command:

```
check_reserve_proof <address> <signature_file> [<message>]
```

Where `<address>` is the address of the wallet where the command get_reserve_proof was ran. `<signiture_file>` is the file that was received from the individual sending you the reserve proof, normally generated as `gyuanx_reserve_proof` and `<message>` is the key set by the individual who sent you the reserve proof.

Therefor for the previous example where we created a reserve proof for 1000 gyuanx and signed with “car”, we would run the command:
  
```
check_reserve_proof T6TmZX8EzZVjS9zNg7zAsrEQFDgcVC2qV2ZMyoWsbyK4SNB2SwMHZtMhPSsFyTmRBQUaGVF5k3qy5CMFM6Lvj7gi3AeszDag7 gyuanx_reserve_proof Car
```
...
For example let’s imagine you have bought a car from a dealership with gyuanx and have sent 1000 gyuanx to the seller. Unfortunately the dealer does not know which transaction is yours as he has received 5 transactions of 1000 gyuanx in the same block for 5 different cars. He knows the txid’s but wants you to prove that you have generate one of the txid’s in his list. Luckily we can prove we generated the txid by using the `get_spend_proof` command.
...
```
signature file saved to: gyuanx_spend_proof
```

This signature file `gyuanx_spend_proof` will be saved in your Gyuanx folder, where your daemon and wallet keys are. Keep in mind every time you run the `get_spend_proof` command it will overwrite your `gyuanx_spend_proof file`.

You will want to send this file to the person who requires the proof. You can upload the gyuanx_spend_proof file through [https://transfer.sh/](https://transfer.sh/) by running the command within the folder of your signature file:

```
curl --upload-file ./gyuanx_spend_proof https://transfer.sh/gyuanx_spend_proof
```
...
```
https://transfer.sh/QhoC7/gyuanx_spend_proof
```

Make sure you provide the following to the individual who will be checking your reserve proof:

-   The `gyuanx_spend_proof` file through the transfer.sh link.

-   The gyuanx transaction txid associated with the transaction you are proving you generated.
    
-   The `<message>` if you encrypted the file.

### 6.2 Checking Spend Proof

To check a spend proof we need to first have the `gyuanx_spend_proof` file in our Gyuanx folder and the txid associated with the transaction being proved.

If you do not have the `gyuanx_spend_proof` file in your gyuanx folder request the individual sending the file to you to use [https://transfer.sh/](https://transfer.sh/), once they send you the link to their `gyuanx_spend_proof` you can use the following command to download it.

```
curl <link> -o gyuanx_spend_proof
```

Replacing `<link>` with the link to download the `gyuanx_spend_proof`.

Now that the `gyuanx_spend_proof` is in our folder we can run the following command:

```
check_spend_proof <txid> <signature_file> [<message>]
```

Where `<txid>` is the txid associated with the transaction that is being proved. `<signiture_file>` is the file that was received from the individual sending you the spend proof, normally generated as `gyuanx_spend_proof` and `<message>` is the key set by the individual who sent you the spend proof.

An example would look like the following command

```
check_spend_proof 20eb3b5545d6587e5a379feb2fc69b43d4f8b6b825bb7eff78e263d4e7e8eaa9 gyuanx_spend_proof car
```
...
```
signature file saved to: gyuanx_tx_proof
```

This signature file `gyuanx_tx_proof` will be saved in your Gyuanx folder, where your daemon and wallet keys are. Keep in mind every time you run the `get_tx_proof` command it will overwrite your `gyuanx_tx_proof` file.

You will want to send this file to the person who requires the proof. You can upload the `gyuanx_tx_proof` file through [https://transfer.sh/](https://transfer.sh/) by running the command within the folder of your signature file:

```
curl --upload-file ./gyuanx_tx_proof https://transfer.sh/gyuanx_tx_proof
```

The terminal will then print out a link to your signature file which you can then provide to the individual performing the check. For example:

```
https://transfer.sh/QhoC7/gyuanx_tx_proof
```

Make sure you provide the following to the individual who will be checking your reserve proof:

-   The `gyuanx_tx_proof` file through the transfer.sh link.

-   The gyuanx transaction txid associated with the transaction you are proving you generated.
    
-   The receivers gyuanx address.
    
-   The `<message>` if you encrypted the file.

### 7.2 Checking tx Proof

To check a tx proof we need to first have the `gyuanx_tx_proof` file in our Gyuanx folder, the receiver's address and the txid associated with the transaction being proved.

If you do not have the `gyuanx_tx_proof` file in your gyuanx folder request the individual sending the file to you to use [https://transfer.sh/](https://transfer.sh/), once they send you the link to their `gyuanx_tx_proof` you can use the following command to download it.

```
curl <link> -o gyuanx_tx_proof
```

Replacing `<link>` with the link to download the `gyuanx_tx_proof`.

Now that the `gyuanx_tx_proof` is in our folder we can run the following command:

```
check_tx_proof <txid> <address> <signature_file> [<message>]
```

Where `<txid>` is the txid associated with the transaction that is being proved, `<address>` is the receiver’s address and `<signiture_file>` is the file that was received from the individual sending you the tx proof, normally generated as `gyuanx_tx_proof` and `<message>` is the key set by the individual who sent you the tx proof.

An example would look like the following command:
```
check_tx_proof 3f8c62b4d83100ff4f89b44a96350e65aeaa83a9b4273c31f94b9aa12e713044 TRrEpWMLd3rRuirYqsjg1iaNsukAAojWjFDhJ2kK2o4uM6tkcjMerA4SZNat6QHEYe1SoGCFQddVPgRqmkA8kARX1ffU1Wcjc gyuanx_tx_proof
```
...
For the previous example we would run the following command from any gyuanx wallet:
...
The terminal will show text of how much Gyuanx the address received. It will also show how many confirmations the transaction has received from the blockchain. For example:
...
The `gyuanx-wallet-cli` allows you to add notes to specific txid’s, however this note does not get stored on the blockchain, rather it is stored on client side, on the device that generates the `tx_note`.
